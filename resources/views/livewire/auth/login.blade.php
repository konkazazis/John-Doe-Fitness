<x-layouts.auth.simple :title="__('Log in')">
    <div class="flex flex-col gap-6">
        <div class="text-center">
            <span class="section-label mb-1 text-center">Welcome back</span>
            <h1 class="font-display text-3xl text-ink uppercase">Log in</h1>
            <p class="mt-2 text-sm text-ink/60">Sign in to your account</p>
        </div>

        @if (session('status'))
            <div class="text-center text-sm font-medium text-sprout">
                {{ session('status') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-4">
            ...
            <div class="flex items-center justify-between px-1 text-sm">
                <label class="inline-flex cursor-pointer items-center gap-2 text-ink/70">
                    <input type="checkbox" name="remember" @checked(old('remember'))
                        class="rounded border-ink/30 text-brand focus:ring-brand">
                    Remember me
                </label>
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="font-semibold text-ink/70 hover:text-brand">
                        Forgot password?
                    </a>
                @endif
            </div>
        ...
        <div class="text-center text-sm text-ink/60">
            ...
            <a href="{{ route('register') }}" class="font-semibold text-ink hover:text-brand">Register</a>
        </div>
    </div>
