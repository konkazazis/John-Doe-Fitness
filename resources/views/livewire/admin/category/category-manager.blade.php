<div class="p-6 lg:p-10">
    ...
    <div class="mb-8 flex items-start justify-between">
        <div>
            <h1 class="text-2xl font-bold text-zinc-800 dark:text-zinc-100">Categories</h1>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Manage post categories.</p>
        </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-zinc-100 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        @if($categories->isEmpty())
            <div class="py-16 text-center text-sm text-zinc-400 dark:text-zinc-500">No categories found.</div>
        @else
                    ...
                    <tbody class="divide-y divide-zinc-50 dark:divide-zinc-800">
                        @foreach($categories as $category)
                            <tr class="transition-colors hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30">
                                <td class="px-6 py-4 font-semibold text-zinc-800 dark:text-zinc-100">{{ $category->name }}</td>
                                <td class="hidden px-4 py-4 font-mono text-xs text-zinc-400 sm:table-cell dark:text-zinc-500">{{ $category->slug }}</td>
                                <td class="hidden px-4 py-4 text-zinc-600 sm:table-cell dark:text-zinc-300">{{ $category->posts_count }}</td>
                                <td class="px-6 py-4">
            ...
            @if($categories->hasPages())
                <div class="border-t border-zinc-100 px-6 py-4 dark:border-zinc-800">{{ $categories->links() }}</div>
            @endif
        @endif
    </div>
    ...
    <div x-data="{ show: @entangle('showModal').live }"
        ...
        style="display: none;">
        <div @click.stop class="w-full max-w-md rounded-2xl border border-zinc-100 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900"
            ...
            <div class="mb-6 flex items-center justify-between">
                <h2 class="text-lg font-bold text-zinc-800 dark:text-zinc-100">{{ $editingId ? 'Edit Category' : 'New Category' }}</h2>
    ...
    <div x-data="{ show: @entangle('deletingId').live }"
        ...
        <div @click.stop class="w-full max-w-sm rounded-2xl border border-zinc-100 bg-white p-6 text-center shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
            <p class="mb-2 text-lg font-bold text-zinc-800 dark:text-zinc-100">Delete category?</p>
            <p class="mb-6 text-sm text-zinc-500">Posts in this category will be uncategorised.</p>
