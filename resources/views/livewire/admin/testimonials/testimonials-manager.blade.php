<div class="p-6 lg:p-10">
    ...
    <div class="mb-8 flex items-start justify-between">
        <div>
            <h1 class="text-2xl font-bold text-zinc-800 dark:text-zinc-100">Testimonials</h1>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Manage your client's testimonials.</p>
        </div>
    ...
    <div class="mb-6">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search testimonials..."
    ...
    <div class="overflow-hidden rounded-2xl border border-zinc-100 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        @if($testimonials->isEmpty())
            <div class="py-16 text-center text-sm text-zinc-400 dark:text-zinc-500">No testimonials yet.</div>
        @else
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="border-b border-zinc-100 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-800/40">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-semibold tracking-wide text-zinc-500 uppercase">Client</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold tracking-wide text-zinc-500 uppercase">Quote</th>
            ...
            @if($testimonials->hasPages())
                <div class="border-t border-zinc-100 px-6 py-4 dark:border-zinc-800">{{ $testimonial->links() }}</div>
            @endif
        @endif
    </div>
    ...
    <div x-data="{ show: @entangle('showModal').live }"
        ...
        <div @click.stop
            ...
            <div class="mb-6 flex items-center justify-between">
                <h2 class="text-lg font-bold text-zinc-800 dark:text-zinc-100">{{ $editingId ? 'Edit Testimonial' : 'New Testimonial' }}</h2>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" wire:model="client" placeholder="Testimonial Client"
                ...
                <div>
                    ...
                    <textarea wire:model="description" rows="3" placeholder="What this testimonial is about…"
                ...
                <div>
                    <label class="mb-1.5 block text-xs font-semibold tracking-wide text-zinc-600 uppercase dark:text-zinc-300">Quote</label>
                    <textarea wire:model="quote" rows="3" placeholder="What is their quote..."
                        class="w-full resize-none rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-800 placeholder-zinc-400 focus:ring-2 focus:ring-zinc-900 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100"></textarea>
                </div>
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" wire:model="is_published" id="test_is_published"
                        class="rounded border-zinc-300 text-zinc-900 focus:ring-zinc-900 dark:border-zinc-600">
                    <label for="test_is_published" class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Published</label>
                </div>
            </div>
            ...
            <div class="mt-6 flex justify-end gap-3 border-t border-zinc-100 pt-4 dark:border-zinc-800">
                ...
                <button wire:click="save" wire:loading.attr="disabled"
                    ...
                    <span wire:loading.remove wire:target="save">{{ $editingId ? 'Save changes' : 'Create testimonial' }}</span>
    ...
    <div x-data="{ show: @entangle('deletingId').live }"
        ...
        <div @click.stop class="w-full max-w-sm rounded-2xl border border-zinc-100 bg-white p-6 text-center shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
            <p class="mb-2 text-lg font-bold text-zinc-800 dark:text-zinc-100">Delete testimonial?</p>
