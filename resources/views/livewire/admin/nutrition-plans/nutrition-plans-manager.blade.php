<div class="p-6 lg:p-10">
    ...
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-zinc-800 dark:text-zinc-100">Nutrition Plans</h1>
        <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Build and assign nutrition plans to your clients.</p>
    </div>
                            ...
                            <button wire:click="selectClient({{ $client->id }})"
                                ...
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    <span class="shrink-0 rounded-full bg-zinc-100 px-2 py-0.5 text-xs font-medium text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">
                                        {{ $client->nutrition_plans_count }} {{ Str::plural('plan', $client->nutrition_plans_count) }}
                                    </span>
                                </div>
                            </button>
                        @endforeach
                            ...
                            <div class="rounded-2xl border border-zinc-100 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <div class="flex shrink-0 items-center gap-1">
                                        ...
                                        <button wire:click="delete({{ $plan->id }})"
                                            wire:confirm="Delete this nutrition plan?"
                                ...
                                <div class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                                    <div class="rounded-lg bg-zinc-50 px-3 py-2 dark:bg-zinc-800/50">
                                        <p class="text-[10px] font-semibold tracking-wide text-zinc-400 uppercase">Calories</p>
                                        <p class="text-sm font-semibold text-zinc-700 dark:text-zinc-200">{{ $plan->daily_calories ?? '—' }}</p>
                                    </div>
                                    <div class="rounded-lg bg-zinc-50 px-3 py-2 dark:bg-zinc-800/50">
                                        <p class="text-[10px] font-semibold tracking-wide text-zinc-400 uppercase">Protein</p>
                                        <p class="text-sm font-semibold text-zinc-700 dark:text-zinc-200">{{ $plan->protein_grams !== null ? $plan->protein_grams.'g' : '—' }}</p>
                                    </div>
                                    <div class="rounded-lg bg-zinc-50 px-3 py-2 dark:bg-zinc-800/50">
                                        <p class="text-[10px] font-semibold tracking-wide text-zinc-400 uppercase">Carbs</p>
                                        <p class="text-sm font-semibold text-zinc-700 dark:text-zinc-200">{{ $plan->carbs_grams !== null ? $plan->carbs_grams.'g' : '—' }}</p>
                                    </div>
                                    <div class="rounded-lg bg-zinc-50 px-3 py-2 dark:bg-zinc-800/50">
                                        <p class="text-[10px] font-semibold tracking-wide text-zinc-400 uppercase">Fat</p>
                                        <p class="text-sm font-semibold text-zinc-700 dark:text-zinc-200">{{ $plan->fat_grams !== null ? $plan->fat_grams.'g' : '—' }}</p>
                                    </div>
                                </div>
                                    ...
                                    <div class="mt-4 divide-y divide-zinc-50 border-t border-zinc-100 dark:divide-zinc-800 dark:border-zinc-800">
                                        @foreach($plan->meals as $meal)
                                            <div class="flex items-center justify-between py-2.5">
                                                <div class="min-w-0">
                                                    <p class="truncate text-sm font-medium text-zinc-700 dark:text-zinc-200">{{ $meal->name }}</p>
                                                    @if($meal->description)
                                                        <p class="truncate text-xs text-zinc-400 dark:text-zinc-500">{{ $meal->description }}</p>
                                                    @endif
                                                </div>
                                                @if($meal->calories !== null)
                                                    <span class="shrink-0 text-xs font-medium text-zinc-500 dark:text-zinc-400">{{ $meal->calories }} kcal</span>
                                                @endif
                                            </div>
                                        @endforeach
                                    </div>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" wire:model="title" placeholder="Cutting Phase - March"
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="goal" placeholder="Lean muscle gain"
                            class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-800 placeholder-zinc-400 focus:ring-2 focus:ring-zinc-900 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100">
                ...
                <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold tracking-wide text-zinc-600 uppercase dark:text-zinc-300">Calories</label>
                        <input type="number" wire:model="daily_calories" placeholder="2200"
                            class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-800 placeholder-zinc-400 focus:ring-2 focus:ring-zinc-900 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100">
                        @error('daily_calories') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold tracking-wide text-zinc-600 uppercase dark:text-zinc-300">Protein (g)</label>
                        <input type="number" wire:model="protein_grams" placeholder="165"
                            class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-800 placeholder-zinc-400 focus:ring-2 focus:ring-zinc-900 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100">
                        @error('protein_grams') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold tracking-wide text-zinc-600 uppercase dark:text-zinc-300">Carbs (g)</label>
                        <input type="number" wire:model="carbs_grams" placeholder="220"
                            class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-800 placeholder-zinc-400 focus:ring-2 focus:ring-zinc-900 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100">
                        @error('carbs_grams') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold tracking-wide text-zinc-600 uppercase dark:text-zinc-300">Fat (g)</label>
                        <input type="number" wire:model="fat_grams" placeholder="70"
                            class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-800 placeholder-zinc-400 focus:ring-2 focus:ring-zinc-900 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100">
                        @error('fat_grams') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div>
                    <div class="mb-2 flex items-center justify-between">
                        <label class="block text-xs font-semibold tracking-wide text-zinc-600 uppercase dark:text-zinc-300">Meals</label>
                        <button type="button" wire:click="addMeal" class="text-xs font-semibold text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100">+ Add meal</button>
                    </div>
                    <div class="space-y-3">
                        @foreach($meals as $i => $meal)
                            <div class="flex items-start gap-2 rounded-lg border border-zinc-100 p-3 dark:border-zinc-800">
                                <div class="grid flex-1 grid-cols-1 gap-2 sm:grid-cols-5">
                                    <input type="text" wire:model="meals.{{ $i }}.name" placeholder="Breakfast"
                                    ...
                                    <input type="text" wire:model="meals.{{ $i }}.description" placeholder="Greek yogurt, oats & berries"
                                        class="rounded-lg border border-zinc-200 bg-white px-3 py-1.5 text-sm text-zinc-800 placeholder-zinc-400 focus:ring-2 focus:ring-zinc-900 focus:outline-none sm:col-span-2 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100">
                                    <input type="number" wire:model="meals.{{ $i }}.calories" placeholder="kcal"
                                ...
                                <button type="button" wire:click="removeMeal({{ $i }})" class="mt-1.5 shrink-0 text-zinc-400 hover:text-red-500">
                                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                </button>
                                @error("meals.{$i}.name") <p class="col-span-full mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                            </div>
                        @endforeach
                        @if(empty($meals))
                            <p class="text-xs text-zinc-400 dark:text-zinc-500">No meals added yet.</p>
                        @endif
                    </div>
                </div>
