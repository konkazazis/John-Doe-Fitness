    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 rounded-2xl border border-zinc-800/70 bg-zinc-900/50">
            <div class="flex items-center justify-between px-6 py-5 border-b border-zinc-800/70">
                <div>
                    <h2 class="font-semibold text-white">Today's workout</h2>
                    <p class="text-sm text-zinc-500">Upper body strength · 45 min</p>
                </div>
                ...
                @foreach([
                    ...
                    <li class="flex items-center justify-between px-6 py-4">
                        <div class="flex items-center gap-3">
                            <span class="w-8 h-8 rounded-lg bg-zinc-800 flex items-center justify-center text-zinc-400">
                            ...
                            <div>
                                <p class="text-sm font-medium text-zinc-200">{{ $exercise['name'] }}</p>
                                <p class="text-xs text-zinc-500">{{ $exercise['sets'] }}</p>
                            </div>
                        </div>
                        <span class="text-sm font-medium text-zinc-300">{{ $exercise['load'] }}</span>
                    </li>
                @endforeach
            </ul>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-2xl border border-zinc-800/70 bg-zinc-900/50 p-6">
                <h2 class="font-semibold text-white mb-4">This week</h2>
                <div class="space-y-2.5">
                    @foreach([
                        ...
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-zinc-500 w-10">{{ $day['day'] }}</span>
                            <span class="flex-1 text-zinc-300">{{ $day['label'] }}</span>
                            @if($day['done'])
            ...
            {{-- Personal bests --}}
            <div class="rounded-2xl border border-zinc-800/70 bg-zinc-900/50 p-6">
                <h2 class="font-semibold text-white mb-4">Personal bests</h2>
                <dl class="space-y-3 text-sm">
                    <div class="flex justify-between">
                        <dt class="text-zinc-500">Bench press</dt>
                        <dd class="text-zinc-200">80 kg</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-zinc-500">Squat</dt>
                        <dd class="text-zinc-200">110 kg</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-zinc-500">Deadlift</dt>
                        <dd class="text-zinc-200">140 kg</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-zinc-500">5k run</dt>
                        <dd class="text-zinc-200">24:12</dd>
                    </div>
                </dl>
            </div>
        </div>
